...
def getCategories():
    ...
    cursor.execute("select category, state, count(distinct business_id) as count "+
                            "from business "+
    ...
    # Step 4: Fetch all rows of the query result
...
def getAttributes():
    ...
    cursor.execute("SELECT state, count(distinct business_id) as total_count FROM business "+
                                            "where state!='BC' group by state order by state")
    total_count_by_states = cursor.fetchall()

    for row in total_count_by_states:
        dict_total = {}
        dict_total["state"] = row[0]
        dict_total["count"] = row[1]
    ...
    for attr in attr_of_interest:
        ...
        cursor.execute("SELECT state, count(distinct business_id) as attr_count FROM business "
                            +"where attr_key = '"+attr+"' and attr_value = 'True' and state!='BC'"
                            +"group by state order by attr_count;")
        count_for_attr_by_state = cursor.fetchall()
